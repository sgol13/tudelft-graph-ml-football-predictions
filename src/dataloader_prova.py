        ...
        for raw_path in tqdm(
            ...
            with open(raw_path, "rb") as f:
                ...
                for raw_match in tqdm(
                    ...
                    events = raw_match["events"]        # COULD INNECESSARY COLUMNS BE DROPPED ALREADY?
                    home_team = raw_match["home_team"]  # COULD ID's BE USED?
                    away_team = raw_match["away_team"]
                    match_id = raw_match["game_id"]

                    graphs_by_team = self._process_match(events)
                    for team, graphs in graphs_by_team.items():
                        for graph in graphs:
                            graph.match_id = match_id
                            graph.home_team = home_team
                            graph.away_team = away_team
                            graph.season = season
                            torch.save(graph, Path(self.processed_dir) / f"data_{idx}.pt")
                            idx += 1
